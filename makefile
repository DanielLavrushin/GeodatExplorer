...
# Linux builds
linux-amd64:
	cd $(SRC_DIR) && wails build -tags webkit2_41 -platform linux/amd64 -ldflags "$(LDFLAGS)" -o $(APP_NAME)
	mkdir -p $(BUILD_DIR)/linux-amd64
	mv $(SRC_DIR)/build/bin/$(APP_NAME) $(BUILD_DIR)/linux-amd64/

linux-arm:
	cd $(SRC_DIR) && wails build -tags webkit2_41 -platform linux/arm -ldflags "$(LDFLAGS)" -o $(APP_NAME)
	mkdir -p $(BUILD_DIR)/linux-arm
	mv $(SRC_DIR)/build/bin/$(APP_NAME) $(BUILD_DIR)/linux-arm/

linux-arm64:
	cd $(SRC_DIR) && wails build -tags webkit2_41 -platform linux/arm64 -ldflags "$(LDFLAGS)" -o $(APP_NAME)
...
# Windows build
windows-amd64:
	cd $(SRC_DIR) && wails build -platform windows/amd64 -ldflags "$(LDFLAGS)" -o $(APP_NAME).exe
...
# macOS builds
darwin-amd64:
	cd $(SRC_DIR) && wails build -platform darwin/amd64 -ldflags "$(LDFLAGS)" -o $(APP_NAME)
	mkdir -p $(BUILD_DIR)/darwin-amd64
	mv $(SRC_DIR)/build/bin/$(APP_NAME) $(BUILD_DIR)/darwin-amd64/

darwin-arm64:
	cd $(SRC_DIR) && wails build -platform darwin/arm64 -ldflags "$(LDFLAGS)" -o $(APP_NAME)
...
# Single native build
build:
	cd $(SRC_DIR) && wails build -tags webkit2_41 -ldflags "$(LDFLAGS)" -o $(APP_NAME)
	mkdir -p $(BUILD_DIR)
